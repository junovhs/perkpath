...
pub fn generate_prompt(itinerary_text: &str, route_types: &[RouteType]) -> String {
    let transport_types: Vec<String> = route_types
        .iter()
        .map(|rt| format!("\"{}\"", rt.id))
        .collect();
    let transport_str = transport_types.join(" | ");

    format!(
        r#"You are a travel itinerary parser. Parse the itinerary and return ONLY valid JSON.
