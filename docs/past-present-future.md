...
1.  **PNG/SVG Export**: Integration with `html2canvas` to allow users to download high-resolution snapshots of their maps for print/web. The ability to add or remove elements from their map globally, and export layers (nodes, arrows, lines, map background etc)
...
**NOTE**: WE FAVOR RUST OVER JAVASCRIPT AT EVERY LEVEL. THE GOAL IS MAXIMAL USE OF RUST/HTML/CSS, AND IDEALLY NONEXISTANT JS.
